...
void AutonSelector::sd_save() {
  ...
  if (selected_routine != nullptr) {
    ...
    char file_data[strlen(selector_name) + strlen(routine_name) + 4];
...
void AutonSelector::sd_load() {
  ...
  for (int id = 0; id < lv_obj_get_child_count(routine_list); id++) {
    ...
    if (list_child == nullptr)
      continue;
    if (strcmp(lv_list_get_button_text(routine_list, list_child), saved_name) !=
        0)
      continue;
...
void AutonSelector::select_cb(lv_event_t *event) {
  lv_obj_t *obj = lv_event_get_target_obj(event);
  AutonSelector::routine_t *routine =
      (AutonSelector::routine_t *)lv_event_get_user_data(event);
  AutonSelector *selector = (AutonSelector *)lv_obj_get_user_data(obj);
  if (selector == nullptr)
    return;
...
void AutonSelector::pg_up_cb(lv_event_t *event) {
  AutonSelector *selector =
      (AutonSelector *)lv_obj_get_user_data(lv_event_get_target_obj(event));
...
void AutonSelector::pg_down_cb(lv_event_t *event) {
  AutonSelector *selector =
      (AutonSelector *)lv_obj_get_user_data(lv_event_get_target_obj(event));
...
void AutonSelector::up_cb(lv_event_t *event) {
  AutonSelector *selector =
      (AutonSelector *)lv_obj_get_user_data(lv_event_get_target_obj(event));
  if (!selector)
    return;
...
void AutonSelector::down_cb(lv_event_t *event) {
  AutonSelector *selector =
      (AutonSelector *)lv_obj_get_user_data(lv_event_get_target_obj(event));
  if (!selector)
    return;
...
AutonSelector::AutonSelector(std::vector<routine_t> autons)
    : AutonSelector("Auton Selector", autons) {}

AutonSelector::AutonSelector(std::string name,
                             std::vector<routine_t> new_routines) {
  this->name = name;
  ...
  for (routine_t &routine : routines) {
    lv_obj_t *new_btn =
        lv_list_add_button(routine_list, NULL, routine.name.c_str());
...
void AutonSelector::next_auton(bool wrap_around) {
  for (int id = 0; id < lv_obj_get_child_count(routine_list); id++) {
    ...
    if (id == lv_obj_get_child_count(routine_list) - 1) {
      if (!wrap_around)
        return;
      // nullptr because the "Nothing" button is always first, and doesnt have
      // user data
      lv_obj_send_event(lv_obj_get_child(routine_list, 0), LV_EVENT_CLICKED,
                        nullptr);
    } else {
...
void AutonSelector::prev_auton(bool wrap_around) {
  ...
  for (int id = 0; id < child_count; id++) {
    ...
    if (id == 0) {
      if (!wrap_around)
        return;
      lv_obj_send_event(lv_obj_get_child(routine_list, child_count - 1),
                        LV_EVENT_CLICKED, &routines[child_count - 1]);
    } else {
...
void AutonSelector::run_callbacks() {
...
void AutonSelector::run_auton() {
  if (selected_routine == nullptr)
    return; // If commanded to do nothing then return
  selected_routine->action();
}

std::optional<AutonSelector::routine_t> AutonSelector::get_auton() {
  if (selected_routine == nullptr)
    return std::nullopt;
  return *selected_routine;
}

void AutonSelector::on_select(AutonSelector::select_action_t callback) {
  select_callbacks.push_back(callback);
}

void AutonSelector::focus() { rd_view_focus(this->view); }
