...
ImageView::ImageView(std::string name, const lv_image_dsc_t *image_src) {
  this->name = name;
  ...
  uint32_t src_h = image_src->header.h;
  uint32_t src_w = image_src->header.w;
  double zoom;
  if (2 * src_h > src_w) {
    zoom = 240.0 / src_h;
  } else {
...
void ImageView::focus() { rd_view_focus(this->view); }
