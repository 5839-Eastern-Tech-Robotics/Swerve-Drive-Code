...
void views_button_cb(lv_event_t *event) {
...
void close_cb(lv_event_t *event) {
	if (lv_obj_get_child_count(alert_cont) > 0) {
		lv_obj_remove_flag(alert_button, LV_OBJ_FLAG_HIDDEN);
	}
...
void alert_button_cb(lv_event_t *event) {
	if (!lv_obj_has_flag(alert_cont, LV_OBJ_FLAG_HIDDEN)) return;
	lv_obj_add_flag(alert_button, LV_OBJ_FLAG_HIDDEN);
...
void alert_cb(lv_event_t *event) {
	...
	lv_obj_t *alert = lv_event_get_target_obj(event);
	lv_obj_delete(alert);
...
void create_ui() {
	...
	lv_obj_t *views_open_button = lv_button_create(screen);
	lv_obj_set_size(views_open_button, 32, 32);
	lv_obj_add_style(views_open_button, &style_core_button, 0);
	lv_obj_add_style(views_open_button, &style_core_button_pr, LV_STATE_PRESSED);
	lv_obj_align(views_open_button, LV_ALIGN_TOP_RIGHT, -4, 4);
	lv_obj_add_event_cb(views_open_button, views_button_cb, LV_EVENT_PRESSED, NULL);

	lv_obj_t *open_image = lv_image_create(views_open_button);
	lv_image_set_src(open_image, &stack);
	lv_obj_set_style_image_recolor(open_image, color_text, 0);
	lv_obj_set_style_image_recolor_opa(open_image, LV_OPA_COVER, 0);
	lv_obj_align(open_image, LV_ALIGN_CENTER, 0, 0);

	alert_button = lv_button_create(screen);
	lv_obj_set_size(alert_button, 32, 32);
	lv_obj_add_style(alert_button, &style_core_button, 0);
	lv_obj_add_style(alert_button, &style_core_button_pr, LV_STATE_PRESSED);
	lv_obj_align(alert_button, LV_ALIGN_TOP_RIGHT, -42, 4);
	lv_obj_add_event_cb(alert_button, alert_button_cb, LV_EVENT_PRESSED, NULL);
	lv_obj_add_flag(alert_button, LV_OBJ_FLAG_HIDDEN);

	lv_obj_t *alert_image = lv_image_create(alert_button);
	...
	lv_obj_t *views_close_button = lv_button_create(view_menu);
	lv_obj_set_size(views_close_button, 32, 32);
	lv_obj_add_style(views_close_button, &style_transp, 0);
	lv_obj_add_style(views_close_button, &style_transp, LV_STATE_PRESSED);
	lv_obj_align(views_close_button, LV_ALIGN_TOP_RIGHT, -4, 4);
	lv_obj_add_event_cb(views_close_button, close_cb, LV_EVENT_PRESSED, NULL);

	lv_obj_t *close_image = lv_image_create(views_close_button);
	lv_image_set_src(close_image, LV_SYMBOL_CLOSE);
...
rd_view_t *rd_view_create(const char *name) {
	...
	view->_list_button = lv_list_add_button(view_list, NULL, name);
	lv_obj_add_style(view->_list_button, &style_core_list_btn, 0);
	lv_obj_add_style(view->_list_button, &style_list_btn_pr, LV_STATE_PRESSED);
	lv_obj_set_user_data(view->_list_button, view);
	lv_obj_add_event_cb(view->_list_button, view_focus_cb, LV_EVENT_PRESSED, view);
	lv_obj_add_event_cb(view->_list_button, close_cb, LV_EVENT_PRESSED, NULL);
...
void rd_view_del(rd_view_t *view) {
	...
	lv_obj_delete(view->_list_button);
