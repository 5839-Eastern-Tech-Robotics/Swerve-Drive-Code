#include "robot/ui/autonSelector.hpp"
...
void AutonSelector::select_cb(lv_event_t *event) {
	...
	AutonSelector::routine_t *routine = (AutonSelector::routine_t *)lv_event_get_user_data(event);
...
void AutonSelector::pg_up_cb(lv_event_t *event) {
	AutonSelector *selector = (AutonSelector *)lv_obj_get_user_data(lv_event_get_target(event));
...
AutonSelector::AutonSelector(std::vector<routine_t> autons) : AutonSelector("Auton Selector", autons) {}

AutonSelector::AutonSelector(std::string name, std::vector<routine_t> new_routines) {
	this->name = name;
